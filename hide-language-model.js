(() => {
  ...
  function renderStarterExplorationHtml(item,context={},esc=(x)=>String(x??'')){
    ...
    const sceneTrail=plan.visual?.length?'<div class="scene-trail" data-progressive-scene>'+plan.visual.map((x,i)=>{const beat=sceneBeat(x);return '<span data-scene-id="'+esc(beat.id)+'" data-scene-step="'+esc(String(i))+'" '+(i?'hidden':'')+'><small>'+esc(String(i+1))+'</small><b>'+esc(beat.label)+'</b></span>'}).join('<i class="scene-link" '+(plan.visual.length>1?'':'hidden')+'>→</i>')+'</div>'+(plan.visual.length>1?'<button class="btn secondary full scene-step-next" type="button" data-next-scene="1">다음 장면 이어보기</button>':''):'';
