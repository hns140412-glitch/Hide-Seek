...
function renderMemorizeStage(){
 ...
 const meaningBlock=role==='NEW'&&thinkingHtml
  ?`<button class="btn primary full" id="confirmNewMeaning" style="margin-top:12px" type="button" disabled>뜻 확인하기</button><div id="newMeaningAnswer" hidden><div class="kor">${esc(w.kor)}</div>${w.example?`<div class="example">${esc(w.example)}</div>`:''}</div>`
 ...
 if($('#confirmNewMeaning'))$('#confirmNewMeaning').onclick=()=>{const answer=$('#newMeaningAnswer');answer.hidden=false;$('#confirmNewMeaning').disabled=true;$('#confirmNewMeaning').textContent='뜻 확인 완료';$('#prepNext').disabled=false;const latestInference=traceList(w,'inference').at(-1);if(latestInference?.event==='FIRST_SEEN_PREDICTION'&&!latestInference.correctedAt){latestInference.finalMeaning=w.kor;latestInference.correctedAt=nowISO()}addWordTrace(w,'acquisition',{event:'MEANING_CONFIRMATION',sheetId:sh.sheetId,stage:'MEMORIZATION',meaningText:w.kor});save()};
