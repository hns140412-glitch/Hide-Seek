...
function renderMemorizeStage(){
 ...
 if(inferenceSubmit)inferenceSubmit.onclick=()=>{const prediction=$('.inference-prediction')?.value.trim()||'';if(!prediction)return toast('정답 보기 전에 네 생각을 한 번 남겨줘.');const clue=$('.inference-clue')?.value||'OTHER',confidence=$('.inference-confidence')?.value||'MEDIUM';addWordTrace(w,'inference',{event:'FIRST_SEEN_PREDICTION',stage:'MEMORIZATION',sheetId:sh.sheetId,predictedConcept:prediction,clueUsed:clue,confidence,recallScoreImpact:false});inferenceSubmit.disabled=true;inferenceSubmit.textContent='추론 기록 완료';if(thinkingReveal){thinkingReveal.disabled=false;thinkingReveal.focus()}save()};
 if(thinkingReveal)thinkingReveal.onclick=()=>{const body=$('.thinking-reveal-body');body.hidden=false;thinkingReveal.disabled=true;thinkingReveal.textContent='구조·장면 단서 확인';const plan=globalThis.HideLanguageModel?.starterExploration?.(w,{encounteredWords});addWordTrace(w,'assistance',{step:plan?.verified?'VERIFIED_ETYMOLOGY_MAP':'THINKING_SCENE',source:plan?.sourceType||'CURATED_SEMANTIC_SUPPORT',sourceRef:plan?.sourceRef||null,stage:'MEMORIZATION',historicalEtymologyClaim:!!plan?.claimsHistoricalEtymology,sheetId:sh.sheetId});if($('#confirmNewMeaning'))$('#confirmNewMeaning').disabled=false;save()};
 const rootStepNext=$('.root-step-next');
 if(rootStepNext)rootStepNext.onclick=()=>{const next=Number(rootStepNext.dataset.nextStep||1),node=$('[data-root-step="'+next+'"]');if(node){node.hidden=false;rootStepNext.dataset.nextStep=String(next+1);addWordTrace(w,'assistance',{step:'ROOT_PROGRESSIVE_REVEAL',stage:'MEMORIZATION',sheetId:sh.sheetId,nodeIndex:next,recallScoreImpact:false});const remaining=$('[data-root-step="'+(next+1)+'"]');if(!remaining){rootStepNext.disabled=true;rootStepNext.textContent='뜻의 흔적 모두 확인'}}save()};
 if($('#confirmNewMeaning'))$('#confirmNewMeaning').onclick=()=>{const answer=$('#newMeaningAnswer');answer.hidden=false;$('#confirmNewMeaning').disabled=true;$('#confirmNewMeaning').textContent='뜻 확인 완료';const latestInference=traceList(w,'inference').at(-1);if(latestInference?.event==='FIRST_SEEN_PREDICTION'&&!latestInference.correctedAt){latestInference.finalMeaning=w.kor;latestInference.correctedAt=nowISO()}addWordTrace(w,'acquisition',{event:'MEANING_CONFIRMATION',sheetId:sh.sheetId,stage:'MEMORIZATION',meaningText:w.kor});$('.inference-outcome-btn').forEach(b=>b.onclick=()=>{const latest=traceList(w,'inference').at(-1);if(latest?.event==='FIRST_SEEN_PREDICTION'){latest.outcome=b.dataset.outcome;latest.outcomeAt=nowISO();latest.assessmentSource='LEARNER_SELF_REPORT';latest.objectiveVerified=false;latest.transferSkillEvidence=true;latest.recallScoreImpact=false}$('.inference-outcome-btn').forEach(x=>x.disabled=true);b.classList.add('selected');$('#prepNext').disabled=false;save()});save()};
