...
function renderMemorizeStage(){
 ...
 const meaningBlock=role==='NEW'&&thinkingHtml
  ?`<button class="btn primary full" id="confirmNewMeaning" style="margin-top:12px" type="button">뜻 확인하기</button><div id="newMeaningAnswer" hidden><div class="kor">${esc(w.kor)}</div>${w.example?`<div class="example">${esc(w.example)}</div>`:''}</div>`
 ...
 if(thinkingReveal)thinkingReveal.onclick=()=>{const body=$('.thinking-reveal-body');body.hidden=false;thinkingReveal.disabled=true;thinkingReveal.textContent='장면 단서 확인';addWordTrace(w,'assistance',{step:'THINKING_SCENE',source:'CURATED_SEMANTIC_SUPPORT',stage:'MEMORIZATION',historicalEtymologyClaim:false,sheetId:sh.sheetId});save()};
 if($('#confirmNewMeaning'))$('#confirmNewMeaning').onclick=()=>{const answer=$('#newMeaningAnswer');answer.hidden=false;$('#confirmNewMeaning').disabled=true;$('#confirmNewMeaning').textContent='뜻 확인 완료';$('#prepNext').disabled=false;addWordTrace(w,'acquisition',{event:'MEANING_CONFIRMATION',sheetId:sh.sheetId,stage:'MEMORIZATION',meaningText:w.kor});save()};
 ...
 $('#prepNext').onclick=()=>{addWordTrace(w,'acquisition',{event:'MEMORIZATION_EXPOSURE',sheetId:sh.sheetId,missionRole:role,thinkingTrailUsed:!!thinkingReveal?.disabled,meaningConfirmed:role!=='NEW'||!thinkingHtml||!!$('#confirmNewMeaning')?.disabled,meaningMapOpened:!$('#prepMeaningMap')?.hidden,sceneShown:!!w.example,sceneText:w.example||'',meaningShown:true,meaningText:w.kor,shapeLength:w.eng.length});S.learning.prepIndex++;if(S.learning.prepIndex>=ws.length){S.learning.prepIndex=0;S.learning.prepCompleted=true;S.learning.phase='first';save();renderFirstContact()}else{save();renderMemorizeStage()}};
