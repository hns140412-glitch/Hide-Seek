...
function makeCodeSession(w){const units=finalSeekUnits(w),n=blankCount(w.eng),idx=shuffle(units.map((_,i)=>i)).slice(0,n).sort((a,b)=>a-b),real=idx.map((i,k)=>({id:`r${k}-${Math.random()}`,ch:units[i],real:true})),pool=shuffle(finalSeekDistractorPool(w,units)),fakeTarget=inferenceWordDomain(w)==='ENGLISH'?Math.max(3,n):Math.min(Math.max(2,n),pool.length),fakes=pool.slice(0,fakeTarget).map((ch,k)=>({id:`f${k}-${Math.random()}`,ch,real:false})),skillProfile=applicableInferenceProfile(w);return {word:w,units,languageDomain:inferenceWordDomain(w),blankIdx:idx,keys:shuffle([...real,...fakes]),answers:{},hintLevel:0,hintPlan:buildMemoryLadder(w),hintPlanSource:skillProfile?.adaptiveClue?{type:'TRANSFERABLE_INFERENCE_HISTORY',bestClue:skillProfile.adaptiveClue.clue,evidenceLevel:skillProfile.evidenceLevel,languageDomain:inferenceWordDomain(w),evidenceBasis:'LEARNER_SELF_REPORT',selfReportedFitRate:skillProfile.adaptiveClue.selfReportedFitRate,currentWordApplicable:skillProfile.currentWordApplicable===true,objectiveVerified:false}:null,hintTrace:[],errorTrace:[],wrongAttempts:0,seconds:S.settings.pressureReduced?24:(units.length<=5?15:units.length<=8?18:21),start:Date.now()}}
