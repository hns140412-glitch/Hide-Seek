...
function memorySceneCue(w){const x=[...traceList(w,'acquisition')].reverse().find(t=>t.event==='EXPOSURE'&&t.sceneShown&&t.sceneText);if(!x)return null;return {text:String(x.sceneText).split(w.eng).join('_____'),source:'PAST_EXPOSURE',sheetId:x.sheetId||''}}
...
function finishCodeRed(){stopCodeTimer();const currentTargets=codeTargets(),resolved=currentTargets.filter(w=>seekAgainResolved(w,S.codeRed.results[w.id])).map(w=>w.id),sh=sheet();if(S.codeRed.retryOnly)S.codeRed.retrace=S.codeRed.retrace.filter(id=>!resolved.includes(id));sh.learningProvenance=sh.learningProvenance||{};sh.learningProvenance.lastActivityAt=nowISO();sh.learningProvenance.lastTargetCount=currentTargets.length;sh.learningProvenance.remainingRetraceCount=S.codeRed.retrace.length;if(S.codeRed.retrace.length){sh.status='RETRACE_REQUIRED';save();return renderRetrace()}sh.caseMastery=100;sh.status='TEST_READY';sh.learningProvenance.completedAt=nowISO();sh.learningProvenance.completedWordCount=validWords().length;sh.learningProvenance.finalSeekResultCount=Object.keys(S.codeRed.results||{}).length;S.learning.phase='done';syncSheetToLexicon(sh);save();renderComplete()}
...
function renderWords(){const ws=validWords(),ordered=[...ws].sort((a,b)=>Number(memoryStatusView(b).priority||0)-Number(memoryStatusView(a).priority||0));$('#view').innerHTML=`<div class="section-title"><h2>내 단어장</h2><span>${ws.length}개 · 원인 기준</span></div><section class="card">${ws.length?`<div class="weak-list">${ordered.map(w=>{const v=memoryStatusView(w);return `<div class="weak-item"><div><b>${esc(w.eng)}</b><div style="font-size:10px;color:#66736a">${esc(w.kor)}${v.strength!=null?` · Memory ${v.strength}%`:''}</div></div><span class="badge ${v.level}">${esc(v.reason.label)}</span></div>`}).join('')}</div>`:'<p>아직 단어가 없어요.</p>'}</section>`;setPartner('단순히 약하다고 부르지 않고, 어떤 기억 흔적이 필요한지 이유를 보여줄게.','note')}
