...
function addLanguageMemoryEvidence(w,evidence={}){
 ...
 return addWordTrace(w,'languageMemory',{languageDomain:domain,objectiveRecall:evidence.objectiveRecall===true,assisted:evidence.assisted===true,recallScoreImpact:evidence.recallScoreImpact!==false,stage:evidence.stage||'',axis:evidence.axis||'',evidenceType:evidence.evidenceType||'',result:evidence.result||'',source:evidence.source||'',...evidence})
}
function languageMemoryEvidenceSummary(w){
 const rows=traceList(w,'languageMemory'),domain=inferenceWordDomain(w),axes={FORM:0,SOUND:0,MEANING:0,RECALL:0,WRITE_OR_RECONSTRUCT:0},objective={FORM:0,SOUND:0,MEANING:0,RECALL:0,WRITE_OR_RECONSTRUCT:0};
 for(const x of rows){const list=Array.isArray(x.axes)?x.axes:[x.axis].filter(Boolean);for(const axis of list){if(axis in axes){axes[axis]++;if(x.objectiveRecall===true)objective[axis]++}}}
 return {languageDomain:domain,total:rows.length,axes,objectiveRecallCounts:objective,last:rows.at(-1)||null}
}
...
function renderMemorizeStage(){
 ...
 $('#speakWord').onclick=()=>{addWordTrace(w,'acquisition',{event:'SOUND_REPLAY',sheetId:sh.sheetId,stage:'MEMORIZATION'});if(inferenceWordDomain(w)==='HANJA')addLanguageMemoryEvidence(w,{axis:'SOUND',evidenceType:'EXPOSURE_ONLY',stage:'MEMORIZATION',source:'SOUND_REPLAY',objectiveRecall:false,recallScoreImpact:false});save();speakWord(w.eng)};
...
function normalizeRecallToken(value,domain='ENGLISH'){
 const normalized=String(value||'').normalize('NFKC').trim().replace(/\s+/g,' ');
 return String(domain||'ENGLISH').toUpperCase()==='ENGLISH'?normalized.toLowerCase():normalized;
}
function renderFirstContact(){
 ...
 const finish=(ok,reason)=>{w.learningStats=w.learningStats||{};if(!ok)w.learningStats.firstRecallWrong=(w.learningStats.firstRecallWrong||0)+1;addWordTrace(w,'retrieval',{result:ok?'FIRST_RECALL_CORRECT':'FIRST_RECALL_WRONG',stage:'FIRST_FIND',reason,sheetId:sh.sheetId,assisted:false});if(inferenceWordDomain(w)==='HANJA')addLanguageMemoryEvidence(w,{axes:['FORM','MEANING','RECALL','WRITE_OR_RECONSTRUCT'],evidenceType:'TYPED_CHARACTER_RECALL',stage:'FIRST_FIND',source:reason,result:ok?'CORRECT':'WRONG',objectiveRecall:true,assisted:false});S.learning.history.push({at:nowISO(),mode:'first',wordId:w.id,result:ok?'CORRECT':'WRONG'});const fb=$('#firstFeedback');fb.hidden=false;fb.textContent=ok?'찾았어요. 기억에서 바로 꺼냈어요.':`정답은 ${w.eng} · 지금은 다시 보고 다음 보강에서 또 찾아요.`;$('#firstNext').disabled=true;$('#firstUnsure').disabled=true;setTimeout(()=>advanceFirst(ok),ok?260:650)};
 $('#firstNext').onclick=()=>{const domain=inferenceWordDomain(w),answer=normalizeRecallToken($('#firstRecallInput').value,domain),target=normalizeRecallToken(w.eng,domain);if(!answer)return toast('떠올린 단어를 입력해 주세요.');finish(answer===target,'TYPED_RECALL')};
...
function hiddenWordStrategy(w){
 ...
 const labels={SEMANTIC_CONTRAST:'뜻 대비',CONFUSION_CONTRAST:'혼동 대비',SOUND_REACTIVATION:'소리 재활성',ORTHOGRAPHIC_SCAFFOLD:'글자 골격',SPACED_RECALL:'간격 회상',LIGHT_REVIEW:'가볍게 확인'};
...
function hiddenWordActivityModel(w,strategy){
 ...
 if(strategy.type==='SOUND_REACTIVATION')return {...base,mode:'SPELL',title:'소리에서 단어 찾기',prompt:'소리를 듣고 단어를 입력해봐.',cue:'SOUND'};
 if(strategy.type==='ORTHOGRAPHIC_SCAFFOLD')return {...base,mode:'SPELL',title:'글자 골격 복원하기',prompt:memoryShapeCue(w.eng),cue:'SHAPE'};
...
function finalSeekUnits(w){const token=String(w?.eng??w??''),domain=typeof w==='object'?inferenceWordDomain(w):(/[A-Za-z]/.test(token)?'ENGLISH':'OTHER');return Array.from(domain==='ENGLISH'?token.toLowerCase():token)}
function finalSeekDistractorPool(w,units=finalSeekUnits(w)){const domain=inferenceWordDomain(w),required=new Set(units),pool=[];if(domain==='ENGLISH')pool.push(...'abcdefghijklmnopqrstuvwxyz');else{for(const x of validWords()){if(x.id===w.id||inferenceWordDomain(x)!==domain)continue;pool.push(...finalSeekUnits(x))}for(const x of Object.values(S.lexicon||{})){const token=String(x?.canonicalSpelling||'').trim();if(!token)continue;const d=String(globalThis.HideLanguageModel?.detectDomain?.({eng:token})||'').toUpperCase();if(d===domain)pool.push(...Array.from(token))}}return [...new Set(pool.filter(ch=>String(ch).trim()&&!required.has(ch)))]}
function blankCount(word){const n=Array.from(String(word||'')).length;return n<=5?Math.min(2,Math.max(1,n-2)):n<=8?3:Math.min(4,n-2)}
...
function makeCodeSession(w){const units=finalSeekUnits(w),domain=inferenceWordDomain(w),n=blankCount(w.eng),idx=shuffle(units.map((_,i)=>i)).slice(0,n).sort((a,b)=>a-b),real=idx.map((i,k)=>({id:`r${k}-${Math.random()}`,ch:units[i],real:true})),pool=shuffle(finalSeekDistractorPool(w,units)),fakeTarget=domain==='ENGLISH'?Math.max(3,n):Math.min(Math.max(2,n),pool.length),fakes=pool.slice(0,fakeTarget).map((ch,k)=>({id:`f${k}-${Math.random()}`,ch,real:false})),distractorSource=domain==='ENGLISH'?'ENGLISH_ALPHABET':fakes.length?'SAME_LANGUAGE_ENCOUNTERS':'NONE',skillProfile=applicableInferenceProfile(w);return {word:w,units,languageDomain:domain,distractorSource,distractorCount:fakes.length,blankIdx:idx,keys:shuffle([...real,...fakes]),answers:{},hintLevel:0,hintPlan:buildMemoryLadder(w),hintPlanSource:skillProfile?.adaptiveClue?{type:'TRANSFERABLE_INFERENCE_HISTORY',bestClue:skillProfile.adaptiveClue.clue,evidenceLevel:skillProfile.evidenceLevel,languageDomain:domain,evidenceBasis:'LEARNER_SELF_REPORT',selfReportedFitRate:skillProfile.adaptiveClue.selfReportedFitRate,currentWordApplicable:skillProfile.currentWordApplicable===true,objectiveVerified:false}:null,hintTrace:[],errorTrace:[],wrongAttempts:0,seconds:S.settings.pressureReduced?24:(units.length<=5?15:units.length<=8?18:21),start:Date.now()}}
...
function renderCodeRed(){const targets=codeTargets();if(S.codeRed.index>=targets.length)return finishCodeRed();const w=targets[S.codeRed.index];codeSession=makeCodeSession(w);selectedKey=null;const letters=codeSession.units.map((ch,i)=>codeSession.blankIdx.includes(i)?`<span class="slot-anchor">_</span>`:esc(ch)).join('');$('#view').innerHTML=`<section class="code-shell"><div class="code-head"><div><span class="code-label">FINAL SEEK</span><div class="code-instruction" style="margin-top:6px">${S.codeRed.retryOnly?'RE-CHECK':'ALL CURRENT WORDS'} · ${S.codeRed.index+1}/${targets.length}</div></div><div id="timeRing" class="time-ring" style="--p:100"><b><span id="timeText">${codeSession.seconds}</span>s</b></div></div><div class="code-word"><div>${letters}</div><div class="slots">${codeSession.blankIdx.map((_,n)=>`<button class="slot" data-slot="${n}" type="button" aria-label="${n+1}번째 빈칸"></button>`).join('')}</div></div><p class="code-instruction">글자 열쇠를 빈칸으로 끌거나, 열쇠를 탭한 뒤 빈칸을 탭하세요.</p><div id="memoryTrace" class="memory-trace" hidden></div><div class="key-tray">${codeSession.keys.map(k=>`<button draggable="true" class="letter-key" data-key="${k.id}" type="button">${esc(k.ch)}</button>`).join('')}</div><div class="btn-row"><button class="btn gold" id="codeHint" type="button">힌트</button><button class="btn secondary" id="undoCode" type="button">되돌리기</button><button class="btn danger" id="passCode" type="button">PASS</button></div></section>`;bindCodeRed();startCodeTimer();setPartner(codeSession.keys.some(k=>!k.real)?'맞는 글자 열쇠만 골라. 다른 글자도 섞여 있어.':'맞는 글자 열쇠를 골라 빈칸을 완성해봐.','radio')}
...
function useCodeHint(){
 ...
 if(step==='SCENE'){const scene=memorySceneCue(w);if(scene){codeSession.hintTrace.at(-1).source=scene.source;codeSession.hintTrace.at(-1).sourceSheetId=scene.sheetId;showMemoryTrace(scene.text,'내가 봤던 장면');setPartner('처음 실제로 봤던 문맥을 다시 꺼내볼게. 정답 글자는 아직 숨겨둘게.','hint');return}}
 if(step==='THINKING_SCENE'){const historical=Object.values(S.lexicon||{}).map(x=>x?.canonicalSpelling).filter(Boolean),current=validWords().filter(x=>x.id!==w.id).map(x=>x.eng),plan=globalThis.HideLanguageModel?.starterExploration?.(w,{encounteredWords:[...new Set([...historical,...current])]});if(plan){const parts=(plan.parts||[]).map(x=>x.label+' = '+x.meaning).join(' · '),text=[plan.scene,parts,plan.links?.length?'전에 만난 연결: '+plan.links.join(', '):''].filter(Boolean).join(' / ');showMemoryTrace(text,'탐험 장면');codeSession.hintTrace.at(-1).source='CURATED_SEMANTIC_SUPPORT';addWordTrace(w,'assistance',{step:'THINKING_SCENE',source:'CURATED_SEMANTIC_SUPPORT',stage:'MEMORY_LADDER',historicalEtymologyClaim:false});setPartner('외울 때 만든 장면부터 다시 꺼내보자. 글자는 아직 보여주지 않을게.','hint');return}}
 ...
 if(step==='MEANING'){showMemoryTrace(w.kor,'의미 흔적');setPartner('뜻은 기억났어. 이제 소리와 글자 모양을 이어보자.','hint');return}
 ...
 if(step==='SHAPE'){showMemoryTrace(memoryShapeCue(w.eng),'모양 흔적');setPartner('전체 글자는 숨겨두고, 글자 골격만 보여줄게.','hint');return}
 ...
 const slot=unanswered[0],needed=codeSession.units[codeSession.blankIdx[slot]],key=codeSession.keys.find(k=>!k.used&&k.ch===needed);if(!key)return;
 showMemoryTrace((slot+1)+'번째 빈칸에 한 글자만 밝혀줄게.','마지막 빛');placeKey(key.id,slot);setPartner('여기까지만 도와줄게. 남은 글자는 네 기억으로 찾아보자.','hint')
}
