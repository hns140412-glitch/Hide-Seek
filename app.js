...
function renderMemorizeStage(){
 ...
 $('#prepNext').onclick=()=>{addWordTrace(w,'acquisition',{event:'MEMORIZATION_EXPOSURE',sheetId:sh.sheetId,missionRole:role,meaningMapOpened:!$('#prepMeaningMap')?.hidden,sceneShown:!!w.example,sceneText:w.example||'',meaningShown:true,meaningText:w.kor,shapeLength:w.eng.length});S.learning.prepIndex++;if(S.learning.prepIndex>=ws.length){S.learning.prepIndex=0;S.learning.prepCompleted=true;S.learning.phase='first';save();renderFirstContact()}else{save();renderMemorizeStage()}};
