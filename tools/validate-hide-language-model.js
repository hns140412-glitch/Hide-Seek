...
assert(basis?.VERSION==='2026.09.21-learning-basis-v2','learning basis version');
...
const unresolvedContext=api.normalizeItem({word:'學',languageDomain:'HANJA',learningContext:{resolved:false,hanjaLevelLabel:'7급'}});
assert(unresolvedContext.learningContext===null,'unresolved learning context must fail closed');
const foreignContext=api.normalizeItem({word:'學',languageDomain:'HANJA',learningContext:{resolved:true,resolvedBy:'HIDE_LOCAL_INFERENCE',hanjaLevelLabel:'7급'}});
assert(foreignContext.learningContext===null,'Hide must not accept self-owned Hanja level inference');
const readyContext=api.normalizeItem({word:'學',languageDomain:'HANJA',learningContext:{resolved:true,resolvedBy:'READY_LEARNING_ENGINE',contextId:'ctx-hanja',learningUnitId:'unit-1',subject:'한자',rangeLabel:'7급 범위',hanjaLevelLabel:'7급',hanjaLevelSchemeRef:'fixture://hanja-grade-scheme',actor_role:'PARENT',child_id:'child-secret'}});
assert(readyContext.learningContext?.hanjaLevelLabel==='7급','resolved Ready Hanja level may be consumed as context');
