(() => {
  ...
  function decodeLearningContext(raw) {
    ...
    try {
      ...
      const list = v => Array.isArray(v)
        ? [...new Set(v.filter(x => typeof x === 'string').map(x => x.trim()).filter(Boolean))].slice(0, 12)
        : [];
  ...
  function resolvedLearningContext() {
    const value = learningContext();
    if (!value) return null;
    return Object.freeze({
      resolutionState: 'RESOLVED',
      resolved: true,
      resolvedBy: 'READY_LEARNING_ENGINE',
      contextId: value.analysis_id,
      learningUnitId: value.learning_unit_id,
      subject: value.subject,
      assignmentRef: value.assignment_id,
      sourceRef: value.analysis_id,
      conceptSkillTarget: value.concept_skill_target,
      activityTypes: [...value.activity_types],
      cognitiveLoadProfile: [...value.cognitive_load_profile],
      confidence: value.confidence,
      unresolvedFlags: [...value.unresolved_flags],
