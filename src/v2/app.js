(() => {
  ...
  function learn(){
    ...
    if(session.stage==='FIRST_FIND'){
      ...
      $('#v2Check').onclick=()=>{const r=HideV2Learning.checkFirstFind(session,m,$('#v2Answer').value);session=r.session;setFlash(r.ok?'기억에서 찾았어요':'다시 연결해볼게요');render()};
      ...
      $('#v2MeaningCheck').onclick=()=>{const r=HideV2Learning.checkMeaning(session,m,$('#v2Meaning').value);session=r.session;render()};
    }else{
      if(w.languageDomain==='KOREAN'){
        ...
        $('#v2DomainDone').onclick=()=>{session=HideV2Learning.submitDomainExtension(session,m,{responseText:$('#v2Response').value}).session;render()};
        ...
        $('#v2DomainDone').onclick=()=>{session=HideV2Learning.submitDomainExtension(session,m,{sound:$('#v2Sound').value}).session;render()};
      }else{
        ...
        $('#v2DomainDone').onclick=()=>{session=HideV2Learning.submitDomainExtension(session,m,{}).session;render()};
      }
    }
  }
