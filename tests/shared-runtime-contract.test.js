...
const hideApp=fs.readFileSync(path.join(__dirname,'..','app.js'),'utf8');
const hideRuntime=fs.readFileSync(path.join(__dirname,'..','hide-runtime.js'),'utf8');
assert(hideApp.includes('TakyVisionIngest'));
assert(hideRuntime.includes('VisionIngest.validateEvidence'));
assert(hideRuntime.includes('pairPrompt'));
console.log('PASS: Hide consumes shared vision ingest evidence mechanics while retaining vocabulary pairing semantics');


const hideAppHttp=fs.readFileSync(path.join(__dirname,'..','app.js'),'utf8');
...
assert(hideAppHttp.includes('HttpJson.request'));
assert(hideAppHttp.includes("'x-goog-api-key':runtimeApiKey"));
